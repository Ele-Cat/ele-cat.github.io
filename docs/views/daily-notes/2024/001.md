...
实现 **「免登录复制 CSDN 代码块」**，[直接安装](https://greasyfork.org/zh-CN/scripts/493135-ele-cat-%E5%85%8D%E7%99%BB%E5%BD%95%E5%A4%8D%E5%88%B6csdn%E4%BB%A3%E7%A0%81%E5%9D%97)：
...
网页版 B 站的视频播放界面，需要长按点赞才可以三连，我们直接在点赞按钮前，加一个真正的「一键三连」功能，[直接安装](https://greasyfork.org/zh-CN/scripts/493083-ele-cat-bilibili%E4%B8%80%E9%94%AE%E4%B8%89%E8%BF%9E)。
...
上两小节中，都是对按钮样式、事件的处理，这节我们实现一个 **「GPT anywhere」**：

```js
// ==UserScript==
// @name         「Ele-Cat」GPT anywhere
// @namespace    https://ele-cat.gitee.io/
// @version      0.0.1
// @description  出神入化-GPT anywhere
// @author       Ele-Cat
// @match        *://*/*
// @grant        GM_addStyle
// @require      https://code.jquery.com/jquery-3.6.0.min.js
...
(function () {
  ...
  GM_addStyle(
    `#svg-button {width: 36px;fill: #fff;pointer-events: none;}.banner-add_container__aadfy, .chat_chat-input-actions__mwYC_, .home_sidebar-reward__VFUk8{display: none !important;}.home_sidebar__fPZfq{width: 230px}.home_window-content__2WGYf{flex: 1;}`
  );
  ...
  $(document).ready(function () {
    ...
    // 创建弹窗
    let gptUrl =
      "https://www.openchatai.site/?access_key=Ib8itwvo9UWwNK/NcR/4%20QtfrWFRzmkUFD5SBj3hWpm8uqV%20%20/oVtLVDD8zXqiZnarYAOLUR7uoXSiZKIggSzQb/x6u%20X4elXK%20tkJQFtgg=&client=pro";
    let $popup = $(
      '<div id="popupContainer" style="display: none; position: fixed; bottom: 50px; left: 50px; z-index: 9998; background: #fff; border: 1px solid #ccc; box-shadow: 2px 2px 5px rgba(0,0,0,0.3);"></div>'
    );
    let $iframe = $(
      `<iframe src="${gptUrl}" style="width: 800px; height: 600px;max-width: 80vw;max-height: 80vh; border: none;"></iframe>`
    );
    let $closeButton = $('<button id="closeButton">关闭</button>');
    $closeButton.css({
      position: "absolute",
      top: "5px",
      right: "5px",
